var starSelect = {
	init: function(){
		...
		$('<div/>').addClass('col s6').attr('id', 'starMin').starRating({initialRating: 0, starSize: 20, disableAfterRate: false}).appendTo($('#starSelectRow'));
		$('<div/>').addClass('col s6').attr('id', 'starMax').starRating({initialRating: 5, starSize: 20, disableAfterRate: false}).appendTo($('#starSelectRow'));
